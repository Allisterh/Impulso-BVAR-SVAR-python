...
Dividing by `s_j` and multiplying by `sigma_i` moves the belief into **contribution space**: one prior standard deviation of `B_exog[i, j]` shifts variable `i` by `exog_prior_scale` of its own AR(1) residual standard deviations when regressor `j` moves by one of its own. That statement is invariant to rescaling either series, which is the property the old prior lacked. `sigma_i` is the scale the *conjugate* Minnesota prior already uses for its lag coefficients, via `minnesota_dummies`. On the PyMC path the two priors do not yet speak in one language: `MinnesotaPrior.build_priors(n_vars, n_lags)` never receives the data, so its `B_sigma` is a pure function of the tightness and decay hyperparameters and carries no `sigma_i` at all. Giving it the same data-dependent scale is a separate change, tracked as a follow-up; adopting that scale here is a step towards it, not a claim that it has happened. (Closed by [ADR-0015](0015-minnesota-cross-lags-scale-with-data.md).) `s_j` is computed on the lag-trimmed rows because those are the rows the likelihood actually sees.
...
- A column that varies over the full sample but is exactly constant once the first `n_lags` rows are trimmed — a dummy that only switches inside the initial conditions — clears `VARData`'s check and then arrives at the prior collinear with the intercept. `_exog_prior_sigma` raises for it. The check is made on the raw standard deviation, *before* the floor: keying the floor off such a column's level would hand a wide prior to an unidentified coefficient, which is exactly the silent failure this ADR is about. The remedy is to drop the column or reduce `lags`.
...
- Keying off `sigma_i` newly exposes the PyMC path to degenerate *endogenous* columns: a constant series gives `sigma_i ≈ 0`, which collapses that whole row of the prior towards zero. `VARData` does not reject constant endogenous columns, and `MinnesotaPrior` does not share the exposure precisely because it ignores the data. The conjugate path has always had it through `minnesota_dummies`. Not guarded here — the same guard belongs with the `MinnesotaPrior` scaling follow-up, where it can be applied once for both terms.
